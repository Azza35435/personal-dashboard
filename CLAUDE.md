...
- **`app/shopping/page.tsx`** — Shopping, tabbed (`?tab=` deep-link, `useSearchParams` in Suspense): 🛒 **Sale Waitlist** (ShoppingWaitlistWidget: watched grocery/pharmacy items with per-item alert rules; "Check now" hits `/api/shopping/check` which scrapes store prices into `shopping_prices`) | 🧺 **Groceries** (GroceriesWidget: shared household list) | ⭐ **Wishlist** (WishlistWidget: private big-ticket purchases).
...
#### PlannerWidget (`components/widgets/PlannerWidget.tsx`)

Full-page day/week time-blocking planner at `/planner` (sidebar entry 🗓 Planner). **Private per user** (own-rows RLS). Sub-components in `components/planner/`; shared maths/palette in `lib/planner.ts` (minutes-from-midnight ints, `snap15`, `minToY`/`yToMin`, `laneLayout` greedy side-by-side lanes, `PLANNER_COLORS` = gym palette). Tables: `planner_blocks` + `planner_routines` (migration 007).

- **Views**: Day (default, the planning surface) | Week (overview) toggle; ‹/›/Today nav on a single `current` Date anchor. Week grid (`WeekGrid.tsx`) is read-only — Mon–Sun columns with per-day "Nh planned" footers; clicking a day/block jumps to Day view.
- **Day timeline** (`DayTimeline.tsx`): 05:00–24:00 canvas (52px/h, scrollable), 15-min snapping, red current-time line. One pointer-drag system (house `[!!drag]` + ref pattern): drag empty canvas → create ghost → inserts block + opens editor; drag block body → move; drag bottom edge → resize; plain click → editor popover. `data-planner-canvas` attribute is the drop target for the todo tray.
- **Editor popover** (in PlannerWidget, TodoWidget-popover style): title, start/end `type=time`, colour swatches, note, delete. Also reachable via the header "+ Block" button (centred, mode `new`).
- **Routine model**: `planner_routines` (title, start_min, duration_min, `days INT[]` 0=Mon…6=Sun, colour, active) render as *virtual* dashed 🔁 blocks — no rows written. Any interaction (tick done / edit / move / "Skip today") *materialises* an override row in `planner_blocks` with `routine_id + date`; `hidden = true` = skipped that day. Render rule: an override row suppresses the virtual block for that date.
- **Todo integration** (`TodoTray.tsx`, right panel): open todos sorted priority→due; drag onto the timeline or tap → place-mode (next canvas click places) → creates a 30-min slate block with `todo_id`. Ticking a linked block also updates `todos.completed` (and block done-state derives `done || linkedTodo.completed` on load). Already-scheduled todos show 🗓.
...
- **Shared grocery list** (Groceries tab on `/shopping`, cyan accent). Items shared through the groups system (`'groceries'` tool) — new items stamped via `firstSharedGroup('groceries')`.
- **Item fields**: name + optional qty (TEXT, freeform) + optional note (behind a "+ note" toggle) + category. Fixed category constant (produce/dairy/meat/bakery/pantry/frozen/household/other) renders aisle-style sections, empty ones skipped. Each row shows the adder's avatar/initial (from `profiles`) and an on-hover × delete.
- **Check-off flow**: checkbox toggles `checked` (optimistic, line-through + dimmed); header shows "N to buy · M in trolley"; **Clear completed** sets `cleared_at = now()` on checked items, moving them to history. `cleared_at IS NULL` = active list.
- **Frequents chips**: computed client-side from history — top 8 by count of `lower(name)`, excluding names already active; tapping re-adds with the most recent qty/category.
- **History**: collapsible, grouped by cleared date, per-row "+ add again".

#### WishlistWidget (`components/widgets/WishlistWidget.tsx`)

- **Private big-ticket wishlist** (Wishlist tab on `/shopping`, violet accent). Own-rows RLS only — not group-shared.
- **Fields**: name, price, URL (auto-`https://`, shown as domain link), priority (red/amber/green dot, sorts the list), `saved_amount` (progress bar toward price; click the "$X saved · N%" label to edit inline), optional target date + occasion (badge: amber ≤30 days, red past).
- **Sections**: Active + collapsible ✓ Purchased (`purchased_at` date shown, "Move back" restores). Header totals: sum saved / sum price of active items.
- **Add/edit**: shared `FormFields` component used by both the "+ Add item" form and per-card ✎ inline edit.
