...
**Sidebar settings panel** (`components/SidebarSettings.tsx`): ⚙ Settings button pinned at the sidebar footer opens a fixed panel (scrim + `fixed z-50` popover, Escape/click-outside closes — same pattern as TodoWidget's edit popover). Sections: **Account** (avatar/name/email from `profiles`, Sign out → `supabase.auth.signOut()` + redirect `/login`), **Sidebar tools** (per tool: ↑/↓ reorder, ✎ inline rename shown as `custom (default)`, 👁/🚫 hide toggle; Dashboard row is pinned with no controls; hidden pages stay reachable by URL), **Reset to defaults** (confirm → deletes the user's `sidebar_prefs` rows), **Admin** (visible when `profiles.is_admin`; member/group management lands in Phase 5). All mutations optimistically update Sidebar state then upsert the full pref list.
